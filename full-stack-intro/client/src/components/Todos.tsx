...
export function Todos() {
  ...
  useEffect(() => {
    async function read() {
      try {
        ...
        const todoRead = await response.json();
        setTodos(todoRead);
      } catch (err) {
        setError(err);
      } finally {
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      const updateTodos = structuredClone(todos);
      updateTodos.push(addedTodo);
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      ...
      const updateTodos = structuredClone(todos);
      const replaceIndex = updateTodos.findIndex(
        (element) => element.todoId === todo.todoId
      );
      updateTodos.splice(replaceIndex, 1, toggled);
      setTodos(updateTodos);
    } catch (err) {
      setError(err);
    }
